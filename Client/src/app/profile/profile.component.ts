...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit() {
    this.sub = this.activatedRoute.params.subscribe(params => {
      ...
      this.http.post("http://localhost:6359/PixoGramSpring/account/getUser", this.user)
        .subscribe((res: User) => {
          this.user = res;
        }
        )
    })
    this.http.post("http://localhost:6359/PixoGramSpring/mymedia/getAll", this.user)
      .subscribe((data: any) => {
        data.forEach(media => {
  ...
  follow(user: User) {
    ...
    this.http.post("http://localhost:6359/PixoGramSpring/follow/follow", this.follower)
  ...
  block(user : User){
    ...
    this.http.post("http://localhost:6359/PixoGramSpring/blocked/block", this.blocked)
