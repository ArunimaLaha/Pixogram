...
export class RegisterComponent implements OnInit {
  ...
  register(){
   /* this.userModel.profilePicture = this.imagePath;
    ...
    this.http.post("http://localhost:6359/PixoGramSpring/account/register",this.userModel)
    .subscribe((data : any) =>{
  ...
  check(){
    this.http.post("http://localhost:6359/PixoGramSpring/account/check",this.userModel.username)
    .subscribe((data : User) =>{
      if(data != null){
