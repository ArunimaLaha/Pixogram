...
export class MediaDetailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.sub = this.activatedRoute.params.subscribe(params => {
      ...
      this.http.post("http://localhost:6359/PixoGramSpring/media/getMedia", this.media)
        .subscribe((res: Media) => {
          this.media = res;
          console.log(this.media);
        }
        )
      this.http.post("http://localhost:6359/PixoGramSpring/media/getComments", this.media)
        .subscribe((res: any) => {
          res.forEach(comment => {
  ...
  postComment() {
    ...
    this.http.post("http://localhost:6359/PixoGramSpring/media/saveComment", this.newComment)
  ...
  like(val: string) {
    ...
    this.http.post("http://localhost:6359/PixoGramSpring/media/like", this.likeObj)
      .subscribe((res: Comment) => {
        if (res != null) {
          if (val == 'Y') {
