...
export class UpdateComponent implements OnInit {
  ...
  check() {
    this.http.post("http://localhost:6359/PixoGramSpring/account/check", this.user.username)
      .subscribe((data: User) => {
        if (data != null) {
  ...
  update() {
    this.http.post("http://localhost:6359/PixoGramSpring/account/update", this.user)
      .subscribe((data: User) => {
